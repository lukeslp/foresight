...
class OpenAIProvider(BaseLLMProvider):
    ...
    def parse_complete(self, messages: List[Message], response_format, **kwargs) -> CompletionResponse:
        """
        Generate a structured completion using GPT with Pydantic model parsing.

        NEW: Uses OpenAI's .parse() method for automatic structured output parsing.

        Args:
            messages: List of Message objects
            response_format: Pydantic BaseModel class defining the expected output structure
            **kwargs: Optional parameters (model, max_tokens, temperature, etc.)

        Returns:
            CompletionResponse with parsed Pydantic model in metadata['parsed']
        ...
        Example:
            ...
            response = provider.parse_complete(
                messages=[Message(role='user', content='Review The Matrix')],
                response_format=MovieReview
            )
            review = response.metadata['parsed']  # MovieReview instance
        """
        formatted_messages = [
            {"role": msg.role, "content": msg.content}
            for msg in messages
        ...
        response = self.client.chat.completions.parse(
            ...
            **{k: v for k, v in kwargs.items() if k not in ["model", "response_format"]}
        )

        message = response.choices[0].message
        parsed_result = message.parsed if hasattr(message, 'parsed') else None

        return CompletionResponse(
            content=message.content or str(parsed_result),
            model=response.model,
            usage={
                "prompt_tokens": response.usage.prompt_tokens,
                "completion_tokens": response.usage.completion_tokens,
    ...
    def json_complete(self, messages: List[Message], **kwargs) -> CompletionResponse:
        """
        Generate a JSON-formatted completion.

        Guarantees the response will be valid JSON using response_format.
        ...
        Returns:
            CompletionResponse with JSON string in content
        """
        formatted_messages = [
            {"role": msg.role, "content": msg.content}
            for msg in messages
